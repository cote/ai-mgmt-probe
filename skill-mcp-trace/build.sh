#!/bin/bash
# Build/package/install for mcp-trace skill.
set -euo pipefail

SKILL_NAME="mcp-trace"
    ...
    cat > "$SBOM" <<EOF
{
  ...
  "metadata": {
    "timestamp": "$TS",
    "tools": [ { "vendor": "the-skill-builder", "name": "build.sh", "version": "1.0" } ],
    "component": {
      "type": "application",
      "bom-ref": "pkg:generic/$SKILL_NAME@2.0",
      "name": "$SKILL_NAME",
      "version": "2.0",
      "description": "Instructions for capturing an end-to-end MCP call trace and rendering a self-contained HTML viewer.",
